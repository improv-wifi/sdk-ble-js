...
Add the following to your website to show a button that start the provisioning process:

```html
<improv-wifi-launch-button></improv-wifi-launch-button>
```

This will render a button if it is supported and a warning message if it is not.
...
<improv-wifi-launch-button>
  <button slot='activate'>Start provision!</button>
